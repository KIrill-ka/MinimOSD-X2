...
static char buf_show[8];
const PROGMEM char compass_pattern[] = 
                        {'\x8b','\x89','\x8a','\x89','\x8a','\x89',
                         '\x8d','\x89','\x8a','\x89','\x8a','\x89',
                         '\x8c','\x89','\x8a','\x89','\x8a','\x89',
                         '\x8e','\x89','\x8a','\x89','\x8a','\x89'};
void setHeadingPatern()
{
  int8_t start;
  start = round((float)osd_heading / (360/24));
  start -= 3;
  if(start < 0) start += 24;
  for(int x=0; x < 7; x++){
    buf_show[x] = pgm_read_byte(compass_pattern+start);
    if(++start > 23) start = 0;
  }
  buf_show[7] = '\0';
}
...
void setHomeVars()
{
  ...
  if((osd_got_home == 0 || (osd_statf & ARMED_F) == 0) && osd_fix_type == 3){
  ...
  if(!osd_got_home) return;

   {
    ...
    bearing = bearing - 180; //absolute return direction
    if(bearing < 0) bearing += 360; //normalization
